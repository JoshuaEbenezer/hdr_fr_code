...
dis_metadata_csv = pd.read_csv(
    "/home/josh-admin/hdr/qa/hdr_chipqa/fall2021_yuv_rw_info.csv")
...
def greed_single_vid(i):
    ...
    dis_filename = os.path.join(
        "/media/josh-admin/seagate/fall2021_hdr_upscaled_yuv", dis_basename)
    print(dis_filename)
...
#    dis_fps = orig_fps
    orig_filename = os.path.join(
        "/media/josh-admin/seagate/fall2021_hdr_upscaled_yuv/", orig_basename)
    ...
    subprocess.check_call(['./run_greed.sh', orig_filename, dis_filename,
                          orig_fps, dis_fps, str(2160), str(3840), str(10), outfolder])


Parallel(n_jobs=-1)(delayed(greed_single_vid)(i)
