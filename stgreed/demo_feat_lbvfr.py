...
def main(args):
    ...
    if (args.dist_fps == args.ref_fps):
        ...
        pseudo_reference_name = os.path.join('./pseudo_reference_lbvfr', os.path.splitext(
            os.path.basename(args.ref_path))[0]+'_pseudo_reference.y4m')
    ...
    for nl_method in ['one_exp']:
        for nl_param in [-0.5, -2, -5, 0.5, 2, 5]:
            for domain in ['local', 'global']:
                outname = os.path.join(args.outfolder, nl_method+'_'+str(
                    nl_param)+'_'+domain, os.path.splitext(os.path.basename(args.dist_path))[0]+'.z')
                outfeatfolder = os.path.join(
                    args.outfolder, nl_method+'_'+str(nl_param)+'_'+domain)
