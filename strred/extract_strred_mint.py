...
import scipy.linalg
from strred.strred_utils import *
...
def extract_info(frame1, frame2):
    ...
    y1 = model.extractSingleBand(
        frame1, filtfile="sp5Filters", band=0, level=4)
    y2 = model.extractSingleBand(
        frame2, filtfile="sp5Filters", band=0, level=4)
...
def compute_strred(referenceVideoData, distortedVideoData):
    ...
    assert C == 1, "strred called with videos containing %d channels. Please supply only the luminance channel" % (
        C,)
...
distorted_yuv = glob.glob(os.path.join(
    '/data/PV_VQA_Study/all_cut_upscaled_hfr_motioninterpolated_yuv_vids', '*'))
...
def single_vid_strred(i):
    ...
    if(dis_fps == fps):
        ...
        ref_video = os.path.join('/data/PV_VQA_Study/all_cut_upscaled_y4m_vids',
                                 content+'_SRC_SRC_SRC_SRC_'+begin_time[:-4]+'.y4m')

    width, height = int(3840), int(2160)
    strred_outname = os.path.join(
        './strred_features_mint/', os.path.splitext(os.path.basename(dis_vid))[0]+'.z')
...
Parallel(n_jobs=80)(delayed(single_vid_strred)(i)
